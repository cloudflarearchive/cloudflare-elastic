...
echo ""
echo "Installing ingest pipeline to $endpoint"
curl -X PUT -u $username:$password "$endpoint/_ingest/pipeline/cloudflare-pipeline" -H 'Content-Type: application/json' -d @cloudflare-ingest-pipeline.json
echo ""
