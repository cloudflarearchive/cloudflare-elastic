...
public class ElasticLambdaSNSForwarder implements RequestHandler<SNSEvent, Void> {
    ...
    private Void handleRequest(S3EventNotification event, Context context) {
    ...
    private void process(RestHighLevelClient es, AmazonS3 s3Client, S3EventNotification event, BulkProcessor processor, String index, String pipeline) {
        if (ELASTIC_DEBUG) {
    ...
    public Void handleRequest(SNSEvent input, Context context) {
        input.getRecords().forEach(record -> {
            handleRequest(S3Event.parseJson(record.getSNS().getMessage()), context);
        });
        return null;
    }
}
